...
//				            self.data = res.result
				 ...
				 * @param {Object} stageId 459 任务ID
				 * @param {Object} tab   TODAY|WEEK|MONTH
				 */
				function getTaskDetailByTab(that,stageId,tab){
					var param = { "stageId": stageId, "userId": 3, "condition":tab};
					$.ajax({
				          url: 'http://10.105.3.165/bj_xcx/planAction/statistics.action',
				          type: 'POST',
				          dataType:'json',
				          ...
				          data:JSON.stringify({parameter:param}),
				          success: function (res) {
							console.log('wgw--getTaskDetailByTab--',res)
							if(res){
								var grid = turnTaskData(res);
//								that.$data.grid = that.$data.grid.concat(grid);
//								that.$data.weekArray = turnDate(res);
								
								for(var i=0;i<that.$data.taskArray.length;i++){
									if(that.$data.taskArray[i].taskId == stageId){
										that.$data.taskArray[i].grid= that.$data.grid.concat(grid);
										that.$data.taskArray[i].weekArray = turnDate(res);
...
//				            self.data = res.result
				...
				function timet(secent){
//					var div = document.getElementById('scrolldIV');
//					div.scrollLeft = div.scrollWidth;
					if(!secent){
						secent = 0.5;
					}
					$("#scrolldIV").scrollLeft(div.scrollWidth*secent);
				}
				function getAllTask(that){

					var param = { "projectId": 255, "userId": 3, "condition":"TODAY"};
					$.ajax({
				          url: 'http://10.105.3.165/bj_xcx/planAction/projectStatistics.action',
				          type: 'POST',
				          dataType:'json',
				          contentType:"application/json; charset=UTF-8",
				          data:JSON.stringify({parameter:param}),
				          success: function (res) {
							console.log('wgw--getAllTask--',res)
							if(res){
								var data = res.list;
								var taskArray = new Array();
								var tasksList = new Array();
								tasksList.push({'taskName':'全部任务','taskId':-1,'isSHow':true});
								for(var i =0; i<data.length;i++){
									var taskitem = data[i];
									var taskItemContent = {};
									taskItemContent.taskName = taskitem.stageName;
									taskItemContent.dateSelect = false;
									taskItemContent.dateSelect = false;
									taskItemContent.taskId = taskitem.id;
									taskItemContent.grid = that.$data.grid.concat(turnTaskData(taskitem.statistics));
									taskItemContent.weekArray = turnDate(taskitem.statistics);
									taskArray.push(taskItemContent);
									
									var taskSelect = {};
									taskSelect.taskName = taskitem.stageName;
									taskSelect.taskId = taskitem.id;
									taskSelect.isSHow = false;
									tasksList.push(taskSelect);
								}
//								that.$data.taskArray = taskArray;
								that.$data.tasksList = tasksList;
								console.log('wgw=====-that.$data.tasksList----',that.$data.tasksList)
							}
							
				          },
				          fail:function(res){
				          	console.log('wgw--getAllTask-fail-',res)
				          }
				        })
					var taskArray = new Array();
					for(var i = 0 ;i<3;i++){
	...
	var vm = new Vue({
	    		...
	    		data:{
	    			...
	    			taskArray:[
	    				{'taskName':'移动开发','taskId':1,'dateSelect':false,'isSHow':false,'grid':'','weekArray':''},
	    				{'taskName':'小程序','taskId':1,'dateSelect':false,'isSHow':true,'grid':'','weekArray':''},
	    				{'taskName':'pc开发','taskId':1,'dateSelect':false,'isSHow':true,'grid':'','weekArray':''},
	    				{'taskName':'图片设计','taskId':1,'dateSelect':false,'isSHow':true,'grid':'','weekArray':''},
...
//	    			getProjectDetail(that);
	    		...
	    		methods:{
	    			...
	    			tabTaskDate:function(tab,taskId){
	    				console.log('wgw-=-tabTaskDate=',tab,taskId);
	    				var that = this;
	    				switch(tab){
	    					case 1:
	    						getTaskDetailByTab(that,taskId,'TODAY');
	    					break;
	    					case 2:
	    						getTaskDetailByTab(that,taskId,'WEEK');
	    					break;
	    					case 3:
	    						getTaskDetailByTab(that,taskId,'MONTH');
	    					break;
	    				}
